 ...
 *
 * @uses tale_travel_header_style()
 */
function tale_travel_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'tale_travel_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/header-image.jpg',
		'default-text-color'     => '181818',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'tale_travel_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'tale-travel' ),
		),
	) );
}
add_action( 'after_setup_theme', 'tale_travel_custom_header_setup' );

if ( ! function_exists( 'tale_travel_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see tale_travel_custom_header_setup().
	 */
	function tale_travel_header_style() {
		...
		wp_add_inline_style( 'tale-travel-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'tale_travel_header_style', 10 );